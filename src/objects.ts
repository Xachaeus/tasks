...
export function isCorrect(question: Question, answer: string): boolean {
    return answer.toLowerCase().replace(/\s/g, '') === question.expected.toLowerCase().replace(/\s/g,'');
}
...
export function isValid(question: Question, answer: string): boolean {
    //console.log(question.type, question.options, `'${answer}'`, question.options.includes(answer) && (question.options.indexOf(answer) === question.options.lastIndexOf(answer)));
    switch (question.type){
        case "short_answer_question":
            return true;
        case "multiple_choice_question":
            return question.options.includes(answer) && (question.options.indexOf(answer) === question.options.lastIndexOf(answer));
        default:
            return false;
    }
}
...
export function duplicateQuestion(id: number, oldQuestion: Question): Question {
    return {...oldQuestion, options: [...oldQuestion.options], name: `Copy of ${oldQuestion.name}`, published: false, id: id};
}
...
export function mergeQuestion(
    ...
    return {...contentQuestion, points: points, id: id, name: name, published: false};
}
